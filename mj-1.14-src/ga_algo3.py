'''
Multiple plays genetic algorithm
'''
import os
import threading
import subprocess
...
def crossover(selected,g):
	...
	rstrat[g] = all_genomes[g][s1]

	#compute mean and variance
	mu = np.mean([all_genomes[g][s1],all_genomes[g][s2]],axis=0)
	sigma = np.std([all_genomes[g][s1],all_genomes[g][s2]],axis=0)
	i=0
	off1 = []
	off2 = []
