...
double *magic2 = &magic[28];
double hysteresis = 4.0;

struct { int ncomps; double values[5]; }
 stratparams[suitness+1] =
 { { 3, { -0.86864059748529199, -0.23804740263535368, 0.4107800436789566 } } , /* chowness */
   { 3, { 0.22528537253336101, 0.59819739583349429, 0.90628310208408136 } } , /* hiddenness */
   { 3, { 0.047261792206415694, 0.36874118911038939, 0.63895674659490553 } } , /* majorness */
   { 3, { 0.66261563663738221, 0.44406361776896014, 0.57302891974563375 } } /* suitness */
 } ;
//--------------------------------------------
void get_inits(double c,double h,double m,double s/*,char *addr*/){
    //our_id = ids;
    //address = malloc(strlen(addr)+1);
    //strcpy(address,addr);
    i_c = c;
    i_h = h;
    i_m = m;
...
void ga_player_run(int ids, char *nm){
  ...
  if ( ! the_game ) exit(1);
  printf("player: %s\n",name );
  printf("chowness: %f\n", stratparams[chowness].values[0]); //i_c
  printf("hiddenness: %f\n", stratparams[hiddenness].values[0]); //i_h
  printf("majorness: %f\n", stratparams[majorness].values[0]); //i_m
  printf("suitness: %f\n",stratparams[suitness].values[0]); //i_s
  // printf("chowness: %f\n", i_c); //i_c
  // printf("hiddenness: %f\n", i_h); //i_h
  // printf("majorness: %f\n", i_m); //i_m
  // printf("suitness: %f\n",i_s); //i_s
  ...
  while ( 1 ) {
   l = get_line(the_game->fd);
    ...
    //printf("%s",l);
    despatch_line(l);
    //if (g_over == 0) return cum_score;
  }
}

int parsefloatlist(const char *fl, stratparamtype spt) {
  char *start, *end;
  int i;
...
int despatch_line(char *line) {
  ...
  case CMsgGameOver:
    //cum_score = the_game->players[the_game->player]->cumulative_score;
    //printf("SCORE: %d",cum_score);
    exit(0);
  case CMsgReconnect:
  ...
  case CMsgNewHand:
    ...
    curstrat.chowness = stratparams[chowness].values[0]; //i_c;//
    curstrat.hiddenness = stratparams[hiddenness].values[0]; //i_h; //
    curstrat.majorness = stratparams[majorness].values[0]; //i_m;//
    curstrat.suitness = stratparams[suitness].values[0]; //i_s;//
    // curstrat.chowness = i_c; //i_c;//
    // curstrat.hiddenness = i_h; //i_h; //
    // curstrat.majorness = i_m; //i_m;//
    // curstrat.suitness = i_s; //i_s;//
    curstrat.suit = 0;
    break;
...
void check_discard(PlayerP p, strategy *strat,int closedkong) {
  PMsgUnion m;
  double bestval,val;
  int canmj;
  char buf[100];
...
double eval(Tile *tp,strategy *strat,double *stratpoints,
       int reclevel,
       int *ninc,
       int *npr,
       double *breadth) {
